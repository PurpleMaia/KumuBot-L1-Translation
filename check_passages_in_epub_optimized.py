...
def check_single_passage(
    ...
    return {
        "index": passage["index"],
        "hawaiian_preview": passage["hawaiian"][:100] + "..."
        if len(passage["hawaiian"]) > 100
        else passage["hawaiian"],
        "english_preview": passage["english"][:100] + "..."
        if len(passage["english"]) > 100
...
def check_passages_in_epub(
    ...
    for csv_path in csv_paths:
        ...
        if num_processes > 1 and len(passages) > 10:
            ...
            batch_size = max(
                1, len(passages) // (num_processes * 4)
            )  # More batches than processes
            batches = [
        ...
        if show_details and found_passages:
            ...
            for p in found_passages[:5]:  # Show first 5
                ...
                if p["hawaiian_found"] and p["english_found"]:
                    print(
                        f"Both found (H: {p['hawaiian_score']:.2f}, E: {p['english_score']:.2f})"
                    )
...
def main():
    ...
    parser.add_argument(
        "epub_path",
        nargs="?",
        help="Path to the epub file (not needed for --compare-csv)",
    )
    parser.add_argument(
        "--csv",
        nargs="+",
    ...
    parser.add_argument(
        "--details", action="store_true", help="Show detailed information about matches"
    )
    ...
    print(
        f"Total passages found across all CSVs: {total_found}/{total_passages} ({total_found / total_passages * 100:.1f}%)"
    )
...
def compare_csv_files(
    ...
    if len(existing_paths) != 2:
        print(
            f"Error: Need exactly 2 CSV files to compare, found {len(existing_paths)}"
        )
        return
    ...
    if show_all or len(common_hashes) <= 10:
        ...
        for i, hash_val in enumerate(sorted(common_hashes)[:10]):
            ...
            hawaiian_preview1 = (
                p1["hawaiian"][:100] + "..."
                if len(p1["hawaiian"]) > 100
                else p1["hawaiian"]
            )
            hawaiian_preview2 = (
                p2["hawaiian"][:100] + "..."
                if len(p2["hawaiian"]) > 100
                else p2["hawaiian"]
            )
            print(
                f"   - {Path(existing_paths[0]).name} Row {p1['index']}: {hawaiian_preview1}"
            )
            print(
                f"   - {Path(existing_paths[1]).name} Row {p2['index']}: {hawaiian_preview2}"
            )
