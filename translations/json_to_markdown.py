    ...
    for file_path in output_path.glob(f"{task_name}_passage_*.json"):
        ...
        if match:
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                if not model_key:
                    for key in data:
                        if key.endswith("_translation") and not key.startswith(
                            "reference_"
                        ):
        ...
        for passage_num in sorted(chapter_passages.keys()):
            ...
            if p.get("special_case") == "grouped_commentary" and "raw_response" in p:
                ...
                commentary_match = re.search(
                    r"<commentary>(.*?)</commentary>", raw_response, re.DOTALL
                )
                if commentary_match:
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert hybrid complex analysis JSON to markdown"
    )
    parser.add_argument(
        "--output-dir", required=True, help="Output directory containing JSON files"
    )
    parser.add_argument(
        "--task-name",
        required=True,
        help="Task name (e.g., hybrid_complex_analysis_enhanced_fewshot)",
    )
    parser.add_argument(
        "--output-file",
        help="Output markdown file path (default: OUTPUT_DIR_TASK_NAME.md)",
    )
    ...
    if not passages:
        print(
            f"Error: No passage files found in {args.output_dir} for task {args.task_name}"
        )
        sys.exit(1)
    ...
    # Report statistics
    total_passages = sum(
        len(chapter_passages) for chapter_passages in passages.values()
    )
    chapters = sorted(passages.keys())

    print(f"Successfully created markdown file: {output_path}")
    print(
        f"Found {total_passages} passages across {len(chapters)} chapter(s): {chapters}"
    )
    print(
        f"Found summaries for chapters: {sorted(summaries.keys()) if summaries else 'None'}"
    )
