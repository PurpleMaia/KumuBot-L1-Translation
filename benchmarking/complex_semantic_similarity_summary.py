...
def calculate_composite_score(results):
    ...
    for component, weight in weights.items():
        if component in results and not np.isnan(results[component]['average_similarity']):
...
def generate_summary_report(output_file="benchmarking/complex_analysis_results.csv"):
    ...
    for model in models:
        ...
        if results:
            ...
            model_data = {
                ...
                'translation_similarity': results.get('translation', {}).get('average_similarity', np.nan),
                'translation_valid': results.get('translation', {}).get('valid_count', 0),
                'commentary_similarity': results.get('commentary', {}).get('average_similarity', np.nan),
                'commentary_valid': results.get('commentary', {}).get('valid_count', 0),
                'summary_similarity': results.get('summary', {}).get('average_similarity', np.nan),
                'summary_valid': results.get('summary', {}).get('valid_count', 0),
            }
            all_results.append(model_data)
    
    # Sort by composite score (descending)
    all_results.sort(key=lambda x: x['composite_score'] if not np.isnan(x['composite_score']) else -1, reverse=True)
    ...
    print("=" * 120)
    print(f"{'Model':<30} | {'Composite':<10} | {'Translation':<12} | {'Commentary':<12} | {'Summary':<10} | {'Valid Passages':<15}")
    print("-" * 120)
    ...
    for result in all_results:
        ...
        print(f"{model:<30} | {composite_str:<10} | {translation_str:<12} | {commentary_str:<12} | {summary_str:<10} | {valid_passages:<15}")
    
    print(f"\nLegend:")
    print(f"  Composite: Weighted average (Translation: 40%, Commentary: 40%, Summary: 20%)")
        ...
        if results:
            ...
            for component, data in results.items():
                ...
                if not np.isnan(similarity):
                    print(f"  {component.capitalize():<12}: {similarity:.4f} ({valid_count}/{valid_count + missing_count} valid)")
                else:
                    print(f"  {component.capitalize():<12}: N/A ({valid_count}/{valid_count + missing_count} valid)")


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description="Generate complex analysis similarity summary")
    parser.add_argument("--detailed", action="store_true", help="Show detailed component comparison")
