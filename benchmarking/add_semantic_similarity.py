...
def main():
    ...
    try:
        ...
        print("Error: dataset_with_similarities.csv not found. Make sure you're in the correct directory.")
        return
    ...
    if new_model not in df.columns:
        ...
        print(f"Note: '{new_model}' column already exists in dataset_with_similarities.csv")
    
    # Check if similarity column already exists
    similarity_column = f"{new_model}_similarity"
    if similarity_column in df.columns:
        print(f"Warning: '{similarity_column}' column already exists. It will be overwritten.")
    ...
    for i, (model_translation, ref_embedding) in enumerate(tqdm(zip(df[new_model], reference_embeddings))):
