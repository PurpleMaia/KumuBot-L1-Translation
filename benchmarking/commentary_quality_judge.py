...
{{
        ...
        for attempt in range(max_retries):
            try:
                response = client.chat.completions.create(
                    model=judge_model,
                    messages=[
                        {"role": "system", "content": "You are an expert evaluator of Hawaiian cultural and linguistic analysis. Provide detailed, objective evaluations in the requested JSON format."},
                ...
                try:
                    ...
                    required_keys = ["score", "justification", "strengths", "weaknesses", "suggestions"]
    ...
    def evaluate_full_commentary(
        ...
        if total_weight > 0:
            ...
            scores = [results["criteria_scores"][c]["score"] for c in self.evaluation_criteria.keys()]
    ...
    def evaluate_model_commentaries(
        ...
        for i, (model_row, ref_row) in enumerate(tqdm(zip(model_df.iterrows(), ref_df.iterrows()), 
            ...
            passage_evaluation["hawaiian_text"] = str(hawaiian_text)[:100] + "..."  # Truncate for storage
    ...
    def _calculate_summary_stats(self, results: Dict[str, any]):
        ...
        for evaluation in results["passage_evaluations"]:
            results["summary_stats"]["overall_scores"].append(evaluation["overall_score"])
            results["summary_stats"]["weighted_scores"].append(evaluation["weighted_score"])
        ...
        for criterion in self.evaluation_criteria.keys():
            ...
            for evaluation in results["passage_evaluations"]:
                if criterion in evaluation["criteria_scores"]:
                    criterion_scores.append(evaluation["criteria_scores"][criterion]["score"])
        ...
        print(f"  Overall Score: {results['summary_stats']['overall_scores_stats']['mean']:.2f} ± {results['summary_stats']['overall_scores_stats']['std']:.2f}")
        print(f"  Weighted Score: {results['summary_stats']['weighted_scores_stats']['mean']:.2f} ± {results['summary_stats']['weighted_scores_stats']['std']:.2f}")
        
        for criterion, stats in results["summary_stats"]["criteria_stats"].items():
            print(f"  {criterion.replace('_', ' ').title()}: {stats['mean']:.2f} ± {stats['std']:.2f}")
    ...
    def save_results(self, results: Dict[str, any], output_dir: str):
        ...
        json_path = f"benchmarking/complex_analysis/{output_dir}_commentary_quality.json"
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False, default=str)
        
        # Create summary CSV
        summary_data = []
        for criterion, stats in results["summary_stats"].get("criteria_stats", {}).items():
            summary_data.append({
                "model": output_dir,
                "criterion": criterion,
        ...
        for eval_result in results["passage_evaluations"]:
            ...
            for criterion in self.evaluation_criteria.keys():
                if criterion in eval_result["criteria_scores"]:
                    row[f"{criterion}_score"] = eval_result["criteria_scores"][criterion]["score"]
                else:
