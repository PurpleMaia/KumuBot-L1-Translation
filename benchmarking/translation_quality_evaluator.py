...
class TranslationQualityEvaluator:
    ...
    def _load_cultural_terms(self) -> List[str]:
        ...
        return [
            "aloha", "mahalo", "pono", "mana", "kapu", "ali'i", "kahuna", 
            "hale", "ohana", "keiki", "kupuna", "malama", "kokua", 
            "hula", "lei", "luau", "poi", "taro", "kalo", "imu",
            "heiau", "konohiki", "mo'i", "kapa", "malo", "pa'u"
        ]
    ...
    def evaluate_cultural_preservation(self, hawaiian_text: str, translation: str) -> float:
        ...
        for term in found_cultural_terms:
            ...
            if (term.lower() in translation_lower or 
                self._has_cultural_explanation(term, translation_lower)):
                preserved_count += 1
    ...
    def evaluate_contextual_accuracy(
        ...
        if any(x is None for x in [hawaiian_embedding, translation_embedding, reference_embedding]):
            return np.nan
        ...
        trans_ref_similarity = self.cosine_similarity(translation_embedding, reference_embedding)
        ...
        if chapter_context and str(chapter_context).strip():
            ...
            if context_embedding is not None:
                ...
                trans_context_similarity = self.cosine_similarity(translation_embedding, context_embedding)
                # Weight: 70% reference similarity + 30% context alignment
                contextual_score = 0.7 * trans_ref_similarity + 0.3 * trans_context_similarity
            else:
    ...
    def evaluate_passage_translations(
        ...
        for i, (model_row, ref_row) in enumerate(tqdm(zip(model_df.iterrows(), ref_df.iterrows()))):
            ...
            cultural_score = self.evaluate_cultural_preservation(hawaiian_text, model_translation)
            results["translation_metrics"]["cultural_preservation"].append(cultural_score)
            ...
            results["translation_metrics"]["contextual_accuracy"].append(contextual_score)
        ...
        for metric_name, scores in results["translation_metrics"].items():
            ...
            if valid_scores:
                ...
                results["summary_stats"][metric_name] = {
                    ...
                    "valid_count": 0, "missing_count": len(scores)
                }
        
        # Print summary
        print("\nTranslation Quality Summary:")
        for metric_name, stats in results["summary_stats"].items():
            print(f"  {metric_name.replace('_', ' ').title()}: {stats['mean']:.4f} ± {stats['std']:.4f}")
    ...
    def save_results(self, results: Dict[str, any], output_dir: str):
        ...
        json_path = f"benchmarking/complex_analysis/{output_dir}_translation_quality.json"
