...
def test_embedding_model(model_name=None, dimensions=None):
    ...
    api_base_url = os.getenv(
        "OPENAI_API_EMBEDDING_BASE_URL", "http://localhost:11434/v1"
    )
    ...
    if not model_name:
        ...
        if not model_name:
            print(
                "❌ Error: No model specified. Set EMBEDDING_MODEL or pass as argument"
            )
            return False
    ...
    try:
        ...
        if dimensions:
            response = client.embeddings.create(
                model=model_name, input=test_texts[0], dimensions=dimensions
            )
        else:
            response = client.embeddings.create(model=model_name, input=test_texts[0])
        ...
        if dimensions:
            batch_response = client.embeddings.create(
                model=model_name, input=test_texts, dimensions=dimensions
            )
        else:
            batch_response = client.embeddings.create(
                model=model_name, input=test_texts
            )
        ...
        print(
            f"✅ Successfully generated {len(batch_embeddings)} embeddings in {elapsed:.3f} seconds"
        )
        print(
            f"⚡ Average time per embedding: {elapsed / len(batch_embeddings):.3f} seconds"
        )
        ...
        all_same_dim = all(
            len(emb) == len(batch_embeddings[0]) for emb in batch_embeddings
        )
        if all_same_dim:
            print(
                f"✅ All embeddings have consistent dimensions: {len(batch_embeddings[0])}"
            )
        else:
        ...
        print(
            f"Normalized: {'Yes' if abs(np.linalg.norm(embedding) - 1.0) < 0.01 else 'No'}"
        )
